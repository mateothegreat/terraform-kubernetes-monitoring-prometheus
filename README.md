...
| Name | Description | Type | Default | Required |
|------|-------------|------|---------|:--------:|
| aws\_resource\_tags | tags for resources created in AWS (i.e.: EBS volumes, etc.) | `map(string)` | `null` | no |
| external\_labels | labels to add to metrics when queried externally | `map(string)` | `null` | no |
| image | https://github.com/prometheus/prometheus/releases | `string` | `"quay.io/prometheus/prometheus:v2.25.2"` | no |
| ingress\_enabled | if enabled creates an ingress mapping at /promtetheus (requires an ingress-controller) | `bool` | `false` | no |
| limit\_cpu | cpu limit | `string` | `"250m"` | no |
| limit\_memory | memory limit | `string` | `"250Mi"` | no |
| loadbalancer\_enabled | if enabled creates a LoadBalancer service to access the prometheus ui/api | `bool` | `false` | no |
| loadbalancer\_internal | if enabled creates a LoadBalancer service to access the prometheus ui/api | `bool` | `false` | no |
| name | prometheus operated name | `string` | n/a | yes |
| namespace | prometheus operated namespace | `string` | n/a | yes |
| node\_selector | labels to determine which node we run on | `map(string)` | `{}` | no |
| password | username to login with (basic auth) | `string` | n/a | yes |
| replicas | number of replicas | `number` | `1` | no |
| request\_cpu | requested cpu | `string` | `"200m"` | no |
| request\_memory | requested memory | `string` | `"200Mi"` | no |
| retention | retention period (i.e.: 6h) | `string` | `"24h"` | no |
| s3\_aws\_access\_key\_id | n/a | `string` | `""` | no |
| s3\_aws\_secret\_access\_key | n/a | `string` | `""` | no |
| s3\_bucket | n/a | `string` | `""` | no |
| s3\_endpoint | n/a | `string` | `"s3.us-east-1.amazonaws.com"` | no |
| scrape\_interval | how often to scrape targets | `string` | `"10s"` | no |
| scrape\_timeout | how long to wait for a scrape to occur | `string` | `"5s"` | no |
| storage\_gb | storage amount for prometheus (minimum 20Gi for AWS) | `number` | `20` | no |
| thanos\_loadbalancer\_enabled | if enabled creates a LoadBalancer service to access the (thanos) prometheus ui/api | `bool` | `false` | no |
| thanos\_loadbalancer\_internal | if enabled creates a LoadBalancer service to access the (thanos) prometheus ui/api | `bool` | `false` | no |
| username | username to login with (basic auth) | `string` | n/a | yes |
